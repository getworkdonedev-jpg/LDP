...
export class SignalConnector implements BaseConnector {
  ...
  private getKey(): string {
    if (this.decryptedKey) return this.decryptedKey;
  ...
  async read(query: string, limit = 10): Promise<Row[]> {
    ...
    try {
      ...
      if (/conversation|contact|chat/.test(q)) {
        rows = db.prepare(
          `SELECT name, active_at FROM conversations WHERE name IS NOT NULL ORDER BY active_at DESC LIMIT ${limit}`
        ).all();
      } else {
        rows = db.prepare(
      ...
      db.close();
      return rows.map((r: any) => ({ ...r, _recency: 0.95, _src: "signal_native" }));
    } finally {
      for (const ext of ["", "-wal", "-shm"]) {
