...
export class SignalConnector implements BaseConnector {
  ...
  private decryptKey(): string {
    ...
    if (prefix !== "v10") throw new Error(`Unsupported prefix '${prefix}'`);
  ...
  async read(query: string, limit = 500): Promise<Row[]> {
    ...
    // Copy WAL if exists
    if (fs.existsSync(this.dbPath + "-wal")) fs.copyFileSync(this.dbPath + "-wal", tmp + "-wal");
    ...
    try {
      ...
      try { fs.unlinkSync(tmp); fs.unlinkSync(tmp + "-wal"); } catch {}
    }
  }
}
