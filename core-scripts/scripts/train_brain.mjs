...
async function sourcePreloaded() {
  ...
  for (const v of Object.values(preloaded)) {
    ...
    if (v.pathPatterns) v.pathPatterns.forEach(p => app.pathPatterns.add(p));
    if (v.tableSignature) v.tableSignature.forEach(p => app.tableSignature.add(p));
...
async function sourceGitHub() {
  ...
  for (const repo of REPOS) {
    try {
      ...
      const files = (data.tree || []).filter(f => /\.(py|js|ts|md|sql)$/.test(f.path));
...
async function sourceHomebrew() {
  ...
  try {
    ...
    for (const cask of Array.isArray(data) ? data : data.casks || []) {
      const name = cask.name?.[0] || cask.token;
      const app = getApp(name);
      app.sources.add("homebrew");
      app.pathPatterns.add(`~/Library/Application Support/${name}/`);
    }
  } catch (e) {
...
async function run() {
  ...
  let data = { version: "2.0", lastUpdated: new Date().toISOString(), apps: [], learned: {} };
  ...
  for (const app of Object.values(GLOBAL_BASE)) {
     ...
     const entry = {
       ...
       confidence: sourceName === "homebrew" ? 0.7 : confidence,
       source: sourceName,
       pathPatterns: Array.from(app.pathPatterns),
       tableSignature: Array.from(app.tableSignature)
     };

     if (!data.learned) data.learned = {};
     const appKey = `ai_${app.appName.toLowerCase().replace(/[^a-z0-9]/g,"_")}`;
     data.learned[appKey] = entry;

     const existingIdx = data.apps.findIndex(a => a.appName === app.appName);
     if (existingIdx >= 0) {
  ...
  data.apps.sort((a, b) => b.confidence - a.confidence);
