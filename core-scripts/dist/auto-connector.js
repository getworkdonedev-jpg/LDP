...
function readSQLiteSchema(filePath) {
    try {
        ...
        if (buf.length < 100 || buf.toString("ascii", 0, 6) !== "SQLite")
            return null;
...
// ── AI schema analyser ────────────────────────────────────────────────────────
async function analyseWithAI(filePath, schema, apiKey, model) {
    // Build a compact schema summary for the prompt
    const schemaSummary = schema.tables.slice(0, 8).map(t => `Table "${t.name}": ${t.columns.slice(0, 12).map(c => `${c.name}(${c.type})`).join(", ")}`).join("\n");
...
export class AutoConnectorGenerator {
     ...
     */
    async generateInteractive(filePath) {
        const result = await this.generateForFile(filePath);
        if (!result) {
...
function heuristicAnalyse(filePath, schema) {
    ...
    if (allCols.some(c => ["url", "visit_count", "last_visit_time", "typed_count"].includes(c)) ||
...
export async function autoGenCLI(apiKey, jsonMode = false) {
    if (!jsonMode) {
