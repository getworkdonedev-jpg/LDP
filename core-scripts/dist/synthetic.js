 ...
 */
// ── Spotify ───────────────────────────────────────────────────────────────────
export class SyntheticSpotifyConnector {
    descriptor = {
    ...
    async read(query, limit = 500) {
        ...
        let rows = this.tracks.map(t => ({ ...t, _recency: Math.random() * 0.7 + 0.3 }));
        if (/focus|work|study/.test(q))
            rows = rows.filter(r => r.focus);
        return rows.sort((a, b) => b.plays - a.plays).slice(0, limit);
    }
}
// ── Banking ───────────────────────────────────────────────────────────────────
const MERCHANTS = [
...
export class SyntheticBankingConnector {
    descriptor = {
    ...
    async read(query, limit = 500) {
        ...
        if (/afford|save|savings|budget/.test(q)) {
            const inc = this.txns.filter(t => t.type === "credit").reduce((s, t) => s + t.amount, 0);
            const exp = this.txns.filter(t => t.type === "debit").reduce((s, t) => s + t.amount, 0);
            return [{
...
// ── Files ─────────────────────────────────────────────────────────────────────
export class SyntheticFilesConnector {
    descriptor = {
    ...
    async read(query, limit = 500) {
        const q = query.toLowerCase();
        let rows = this.docs.map(d => ({ ...d, _recency: 0.8 }));
        if (/contract|legal|nda|clause/.test(q))
            rows = rows.filter(r => r.type === "contract");
        else if (/salary|pay|payslip/.test(q))
            rows = rows.filter(r => ["payslip", "finance"].includes(r.type));
        else if (/health|medical/.test(q))
            rows = rows.filter(r => r.type === "health");
        return rows.slice(0, limit);
    }
}
// ── WhatsApp ──────────────────────────────────────────────────────────────────
export class SyntheticWhatsAppConnector {
    descriptor = {
    ...
    async read(query, limit = 500) {
        ...
        return rows.sort((a, b) => b.message_count - a.message_count).slice(0, limit);
    }
}
export function registerAllSynthetic(engine) {
