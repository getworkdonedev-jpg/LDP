...
async function start() {
    ...
    console.log("\n  \x1b[1mLDP — Local Data Protocol\x1b[0m v3.1.0");
...
async function connect(app) {
    ...
    if ("error" in info) {
        console.error(`  \x1b[31m✗\x1b[0m ${info.error}`);
        return;
    }
    ...
    if (!["yes", "y"].includes(ans.trim().toLowerCase())) {
...
function audit() {
    const entries = new LDPEngine().audit.readLog();
