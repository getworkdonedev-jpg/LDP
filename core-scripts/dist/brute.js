...
async function brute() {
    const _sqlcipher = await import("@journeyapps/sqlcipher");
    ...
    for (const pragmas of configs) {
        ...
        try {
            const res = await new Promise((resolve, reject) => {
                const db = new sqlcipher.Database(tmp, sqlcipher.OPEN_READONLY, (err) => {
                    if (err)
                        return reject(err);
                    let i = 0;
                    function next() {
                        if (i >= pragmas.length) {
                            db.all("SELECT count(*) FROM messages", (err2, res) => {
                                db.close();
                                if (err2)
                                    reject(err2);
                                else
