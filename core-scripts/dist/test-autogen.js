...
function writeFakeSQLite(dir, name, createSQL) {
...
async function runTests() {
    ...
    await test("Chrome path detected via fingerprint", async () => {
        ...
        writeFakeSQLite(path.dirname(fakePath), "History", `CREATE TABLE urls (id INTEGER PRIMARY KEY, url TEXT, title TEXT, visit_count INTEGER, typed_count INTEGER, last_visit_time INTEGER);
    ...
    await test("WhatsApp path detected via fingerprint", async () => {
        ...
        const fakePath = writeFakeSQLite(wsDir, "ChatStorage.sqlite", `CREATE TABLE ZWAMESSAGE (Z_PK INTEGER, ZTEXT TEXT, ZFROMJID TEXT, ZTOJID TEXT, ZMESSAGEDATE REAL);
    ...
    await test("Spotify path detected via fingerprint", async () => {
        ...
        const fakePath = writeFakeSQLite(spDir, "podcasts.db", `CREATE TABLE playback_history (id INTEGER, track_uri TEXT, play_count INTEGER, ms_played INTEGER, ts_played INTEGER);`);
        const gen = new AutoConnectorGenerator({ extraPaths: [tmpDir] });
        const result = await gen.generateForFile(fakePath);
    ...
    await test("Browser history schema → heuristic detection", async () => {
        ...
        const fakePath = writeFakeSQLite(fakeDir, "webdata.db", `CREATE TABLE urls (id INTEGER, url TEXT NOT NULL, title TEXT, visit_count INTEGER DEFAULT 0, typed_count INTEGER DEFAULT 0, last_visit_time INTEGER NOT NULL);
    ...
    await test("Messaging schema → heuristic detection", async () => {
        ...
        const fakePath = writeFakeSQLite(msgDir, "messages.db", `CREATE TABLE messages (message_id INTEGER PRIMARY KEY, sender TEXT, recipient TEXT, body TEXT, timestamp INTEGER);
    ...
    await test("Finance schema → heuristic detection", async () => {
        ...
        const fakePath = writeFakeSQLite(finDir, "accounts.db", `CREATE TABLE transactions (id INTEGER, amount REAL, merchant TEXT, account_id INTEGER, balance REAL, date INTEGER);
    ...
    await test("Calendar schema → heuristic detection", async () => {
        ...
        const fakePath = writeFakeSQLite(calDir, "events.db", `CREATE TABLE events (event_id INTEGER, summary TEXT, dtstart INTEGER, dtend INTEGER, location TEXT);`);
        const gen = new AutoConnectorGenerator({ extraPaths: [tmpDir] });
        const result = await gen.generateForFile(fakePath);
    ...
    await test("Notes schema → heuristic detection", async () => {
        ...
        const fakePath = writeFakeSQLite(notesDir, "notes.db", `CREATE TABLE notes (note_id INTEGER, title TEXT, content TEXT, created_at INTEGER, modified_at INTEGER);`);
        const gen = new AutoConnectorGenerator({ extraPaths: [tmpDir] });
        const result = await gen.generateForFile(fakePath);
    ...
    await test("System cache dirs are skipped in scan", async () => {
        ...
        writeFakeSQLite(cacheDir, "cache.db", `CREATE TABLE urls (id INTEGER, url TEXT, visit_count INTEGER);`);
        const gen = new AutoConnectorGenerator({ extraPaths: [tmpDir] });
    ...
    await test("Generated connector has valid descriptor shape", async () => {
        ...
        const fakePath = writeFakeSQLite(testDir, "history.db", `CREATE TABLE urls (id INTEGER, url TEXT, visit_count INTEGER, last_visit_time INTEGER);`);
        const gen = new AutoConnectorGenerator({ extraPaths: [tmpDir] });
        const result = await gen.generateForFile(fakePath);
    ...
    await test("Generated connector.read() returns rows", async () => {
        ...
        const fakePath = writeFakeSQLite(testDir2, "msgs.db", `CREATE TABLE messages (message_id INTEGER, sender TEXT, body TEXT, timestamp INTEGER);`);
        const gen = new AutoConnectorGenerator({ extraPaths: [tmpDir] });
        const result = await gen.generateForFile(fakePath);
        assert(result !== null, "Should generate result");
    ...
    await test("connector.connect() succeeds when file exists", async () => {
        ...
        const fakePath = writeFakeSQLite(testDir3, "data.db", `CREATE TABLE events (id INTEGER, summary TEXT, dtstart INTEGER);`);
        const gen = new AutoConnectorGenerator({ extraPaths: [tmpDir] });
        const result = await gen.generateForFile(fakePath);
        assert(result !== null, "Should generate result");
    ...
    await test("connector.connect() throws when file missing", async () => {
        ...
        writeFakeSQLite(tmpDir, "nonexistent_after_scan.db", `CREATE TABLE urls (id INTEGER, url TEXT, visit_count INTEGER);`);
        const result = await gen.generateForFile(fakePath);
