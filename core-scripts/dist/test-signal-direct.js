...
function decryptSignalKey() {
    console.log("\n[Signal] Requesting Keychain access for 'Signal Safe Storage'...");
    console.log("[Signal] If a dialog appears, click Allow.\n");
...
async function main() {
    ...
    try {
        ...
        console.log("\n[Signal] Tables found:", tables.map((r) => r.name).join(", "));
        // Fetch recent messages
        const messages = db.prepare(`SELECT _id, body, dateReceived, conversationId
       FROM messages
       ...
       LIMIT 10`).all();
        ...
        for (const msg of messages) {
            ...
            const body = msg.body.substring(0, 80);
